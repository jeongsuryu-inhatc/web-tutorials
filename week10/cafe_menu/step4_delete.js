// 단계 6 (step4): 메뉴 삭제 — Delete (CRUD 완성본)
// 실행: node step4_delete.js → http://localhost:3000/menus
...
// [D] 삭제: POST /menus/:id/delete → DELETE
app.post("/menus/:id/delete", (req, res) => {
  db.prepare("DELETE FROM menus WHERE id = ?").run(req.params.id);
  res.redirect("/menus");
});

app.listen(PORT, () => {
  console.log(`카페 메뉴 서버(CRUD 완성) 실행 중: http://localhost:${PORT}/menus`);
});
